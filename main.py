    ...
    <body class="bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900 text-white min-h-screen">
        ...
        <div id="help-modal" class="modal">
            <div class="modal-content">
                ...
                <div class="text-sm space-y-2"><p><strong>1.</strong> Зарегистрируйся или войди.</p><p><strong>2.</strong> Опиши шаблон.</p><p><strong>3.</strong> Нажми «Создать».</p><p><strong>4.</strong> Сохрани, скачай или скопируй HTML.</p></div>
            </div>
        </div>
        <div id="about-modal" class="modal">
            <div class="modal-content">
                ...
                <div class="text-sm space-y-2"><p><strong>SiteForge</strong> — генератор HTML-шаблонов с помощью ИИ.</p><p>Создаём красивые заготовки за секунды.</p><p class="text-gray-400 mt-3">Версия: 1.0 | Сделано с ❤️</p></div>
            </div>
        </div>
        ...
        <div class="max-w-2xl w-full px-4 mx-auto py-6">
            ...
            <div class="bg-gray-900 rounded-2xl p-8 border border-gray-800 mt-4 relative">
                ...
                <div id="pli" style="display:none;">
                    ...
                    <div class="text-center mt-6"><button onclick="logout()" class="text-red-400 hover:text-red-300 text-sm">Выйти из аккаунта</button></div>
                </div>
            </div>
        </div>
        <script>
...
def auth_page():
    ...
    <head>
        ...
        <style>
            ...
            .btn-primary { background: linear-gradient(135deg, #6366f1, #8b5cf6); color: white; padding: 14px; border-radius: 14px; font-weight: bold; border: none; width: 100%; cursor: pointer; }
    ...
    <body class="bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900 text-white min-h-screen">
        <div class="max-w-md w-full px-4 mx-auto py-12">
            ...
            <div class="bg-white/5 backdrop-blur-lg rounded-2xl p-6 border border-white/10">
                <div id="afl">
                    ...
                    <input id="le" type="email" placeholder="Email" class="input-field">
                    <input id="lp" type="password" placeholder="Пароль" class="input-field">
                    <button onclick="login()" class="btn-primary mb-3">Войти</button>
                    <p class="text-xs text-gray-400 text-center mt-2">Нет аккаунта? <a href="#" onclick="showReg()" class="text-white font-bold hover:underline">Зарегистрироваться</a></p>
                </div>
                <div id="afr" style="display:none;">
                    <h3 class="text-sm font-bold mb-4">Регистрация</h3>
                    <input id="re" type="email" placeholder="Email" class="input-field">
                    <input id="rp" type="password" placeholder="Пароль" class="input-field">
                    <input id="rp2" type="password" placeholder="Подтвердите пароль" class="input-field">
                    <button onclick="register()" class="btn-primary mb-3">Зарегистрироваться</button>
                    <p class="text-xs text-gray-400 text-center mt-2">Уже есть аккаунт? <a href="#" onclick="showLog()" class="text-white font-bold hover:underline">Войти</a></p>
                </div>
        ...
        <script>
            ...
            async function login() {
                const e = document.getElementById('le').value, p = document.getElementById('lp').value, s = document.getElementById('as');
                if (!e || !p) { s.textContent = 'Заполни все поля'; return; }
                try {
                    const r = await fetch('/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: e, password: p }) });
                    if (!r.ok) { const d = await r.json(); s.textContent = '❌ ' + d.detail; }
                    else {
                        const u = await r.json();
                        localStorage.setItem('siteforge_user', JSON.stringify(u));
                        localStorage.setItem('siteforge_pass', p);
                        s.textContent = '✅ Вход выполнен!';
                        setTimeout(() => { window.location.href = '/'; }, 1000);
                    }
                } catch(e) { s.textContent = '❌ Ошибка: ' + e.message; }
            }
            async function register() {
                const e = document.getElementById('re').value, p = document.getElementById('rp').value, p2 = document.getElementById('rp2').value, s = document.getElementById('as');
                if (!e || !p || !p2) { s.textContent = 'Заполни все поля'; return; }
                if (p !== p2) { s.textContent = '❌ Пароли не совпадают'; return; }
                if (p.length < 4) { s.textContent = '❌ Пароль от 4 символов'; return; }
                try {
                    const r = await fetch('/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: e, password: p }) });
                    const d = await r.json();
                    if (!r.ok) { s.textContent = '❌ ' + d.detail; }
                    else {
                        s.textContent = '✅ Регистрация успешна! Теперь войдите.';
                        showLog();
                        document.getElementById('le').value = e;
                    }
                } catch(e) { s.textContent = '❌ Ошибка: ' + e.message; }
            }
        </script>
