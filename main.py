...
def send_verification_email(email: str, token: str):
    ...
    verify_url = f"https://ai-site-builder-public-production.up.railway.app/verify?email={email}&token={token}"
    msg = MIMEText(f"Привет!\n\nПодтверди email для SiteForge:\n{verify_url}\n\nЕсли ты не регистрировался, игнорируй это письмо.", "plain", "utf-8")
    msg["Subject"] = "Подтверждение email — SiteForge"
    msg["From"] = smtp_user
    msg["To"] = email
...
def init_db():
    ...
    """)
    # Добавим колонки, если их нет (для существующих баз)
    try:
        cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS email_verified BOOLEAN DEFAULT FALSE")
        cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS verification_token VARCHAR(64)")
    except:
        pass
    conn.commit()
    cur.close()
...
def verify_email(email: str, token: str):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM users WHERE email = %s AND verification_token = %s", (email, token))
    user = cur.fetchone()
    if user:
        cur.execute("UPDATE users SET email_verified = TRUE, verification_token = NULL WHERE email = %s", (email,))
        conn.commit()
        cur.close()
        conn.close()
        return HTMLResponse("<h2>✅ Email подтверждён! <a href='/'>Вернуться на сайт</a></h2>")
    cur.close()
    conn.close()
    return HTMLResponse("<h2>❌ Неверная ссылка</h2>")
...
def register(req: AuthRequest):
    ...
    return {"message": "Регистрация успешна. Проверьте почту для подтверждения email.", "user_id": user_id}
        ...
        <div class="max-w-2xl w-full px-4 mx-auto py-6">
            <div class="flex justify-between items-center mb-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="/profile" id="top-avatar-link" style="display:none;"><div class="top-avatar-wrapper"><div class="avatar" id="avatar-icon"></div><span class="top-email" id="avatar-email"></span></div></a>
                </div>
            </div>
            <div class="text-center mb-8"><div class="text-5xl mb-2">🚀</div><h1 class="text-4xl font-extrabold bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">SiteForge</h1>
            <div class="flex justify-center gap-3 mt-2"><button onclick="openModal('help')" class="text-xs text-gray-500 hover:text-gray-300">Помощь</button><button onclick="openModal('about')" class="text-xs text-gray-500 hover:text-gray-300">О нас</button></div></div>
            <div class="bg-white/5 backdrop-blur-lg rounded-2xl p-6 border border-white/10 shadow-2xl">
                <input id="desc" type="text" placeholder="💡 Опиши шаблон" class="input-field" maxlength="500">
            ...
            <div id="preview-container"><div class="flex justify-between items-center mb-3 flex-wrap gap-2"><span class="text-sm text-gray-300">Предпросмотр</span><div class="flex gap-1 flex-wrap"><button onclick="saveToGallery()" class="text-xs bg-gray-700 hover:bg-gray-600 text-white px-3 py-1.5 rounded-lg">💾</button><button onclick="downloadHTML()" class="text-xs bg-gray-700 hover:bg-gray-600 text-white px-3 py-1.5 rounded-lg">📥</button><button onclick="copyCode()" class="text-xs bg-gray-700 hover:bg-gray-600 text-white px-3 py-1.5 rounded-lg">📋</button><button onclick="closePreview()" class="text-gray-500 hover:text-red-400 text-lg px-2">✕</button></div></div><iframe id="preview-frame"></iframe></div>
...
def profile_page():
    ...
    <body class="bg-gray-950 text-white min-h-screen">
        <div class="max-w-2xl w-full px-4 mx-auto py-6">
            ...
            <div class="bg-gray-900 rounded-2xl p-8 border border-gray-800 mt-4">
                ...
                <div id="pli" style="display:none;">
                    ...
                    <button onclick="logout()" class="fixed bottom-6 right-6 text-red-400 hover:text-red-300 text-sm bg-gray-800 px-3 py-1.5 rounded-lg">Выйти</button>
                </div>
            </div>
        </div>
        <script>
    ...
    <body class="bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900 text-white min-h-screen">
        <div class="max-w-md w-full px-4 mx-auto py-12">
            ...
            <div class="bg-white/5 backdrop-blur-lg rounded-2xl p-6 border border-white/10">
                ...
                <p id="as" class="mt-3 text-xs text-center text-gray-400"></p>
            </div>
        </div>
        <script>
